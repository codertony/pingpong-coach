...
4. **规则未审核时禁止输出达标/调整结论**，只能 `observation_only`。这条约束必须写在代码路径的**前面**。
   "已审核"**不是一个字段**：要 status 是 reviewed，**且**审核人／来源／许可／适用条件／参考片段 id
   五项齐备（缺一项就按未审核处理，并写进 `limitations` —— 静默降级与"当成审过了"结果一样）。
   见 `apps/api/src/coach/knowledge.ts` 的 `reviewedClaimDefects`。
