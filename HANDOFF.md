...
**最近一次全量验证**：`pnpm verify` 退出码 0（393 项单元测试 + 依赖体积预算），`pnpm test:e2e` 53 passed（1 项按需 skip）
...
| 后端全链路（含 mock 与真实模型契约） | api 141 项测试（含 1 项按需 skip 的真实模型烟雾测试）|
| 前端采集/证据/播报/组件渲染/配色对比度 | web 105 项 vitest（含 jsdom + Testing Library 渲染测试） |
| **浏览器真实行为** | **web 71 项 Playwright（真实 Chromium）** |
| **活链路（真人画面）** | `live-capture.e2e.ts`：文件当摄像头，**整条采集链路跑在真人帧上**（见下方"未验证"栏） |
...
| 项 | 为什么未验证 |
| --- | --- |
| **骨架是否贴合关节** | 🟡 **已在一支真实素材上目视确认贴合**（7 个采样帧，含持拍侧手臂局部放大；逐帧检出 244/244）；**且活链路上也验证过**（文件当摄像头，产品自己的采集链路在真人帧上画出了骨架与准备区，`live-capture.e2e.ts`）。但**只有一个人、一个机位**，判定者是看图模型 → **你自己机位上仍未验，F-006 保持 OPEN** |
...
| 真实多模态模型的质量/延迟/费用 | 🟡 **已接上真实模型测过一轮**（DeepSeek `deepseek-flash`，2026-09-16）：链路通、`mock:false`、图片**确实到达模型**（prompt_tokens 35→224）、模型**自发遵守红线**（observation_only / 腕峰≠击球 / 不推断发力）、服务端校验实跑（`rejectedClaims: []`）。延迟中位 **3928ms**。**但"建议质量"与费用未验证** —— 前者要人读，后者未统计 |
...
> 🔴 **最重要的一句话**：
> 测试从 172 涨到 564（493 单元 + 71 e2e），但**增量几乎全部落在"代码正确性"上**。
...
| ❌ 声称"测试全绿所以识别准确" | 564 项测试（493 单元 + 71 e2e）里没有一项验证识别准确率 |
...
| `pnpm test` | 只跑单元测试（493 项） |
| `pnpm test:e2e` | 只跑浏览器测试（71 项，真实 Chromium；会自起 vite + 一个真实 API 进程） |
...
> 代码骨架完整，工程护栏齐备，**493 项单元测试 + 71 项浏览器测试全部通过**。
