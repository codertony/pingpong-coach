...
| 几何/滤波/切分/特征/规则/准备区标定/手部几何 | motion-core 176 项测试 |
| 后端全链路（含 mock） | api 135 项测试 |
| 前端采集/证据/播报/组件渲染/配色对比度 | web 60 项 vitest（含 jsdom + Testing Library 渲染测试） |
...
> 🔴 **最重要的一句话**：
> 测试从 172 涨到 467（405 单元 + 54 e2e），但**增量几乎全部落在"代码正确性"上**。
...
| ❌ 声称"测试全绿所以识别准确" | 467 项测试（413 单元 + 54 e2e）里没有一项验证识别准确率 |
...
| `pnpm test:e2e` | 只跑浏览器测试（54 项，真实 Chromium；会自起 vite + 一个真实 API 进程） |
...
> 代码骨架完整，工程护栏齐备，**413 项单元测试 + 54 项浏览器测试全部通过**。
