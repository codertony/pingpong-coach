...
> 🔴 **最重要的一句话**：
> 测试从 172 涨到 446（393 单元 + 52 e2e），但**增量几乎全部落在"代码正确性"上**。
...
| ❌ 声称"测试全绿所以识别准确" | 446 项测试（393 单元 + 53 e2e）里没有一项验证识别准确率 |
...
| `pnpm test:e2e` | 只跑浏览器测试（53 项，真实 Chromium；会自起 vite + 一个真实 API 进程） |
...
> 代码骨架完整，工程护栏齐备，**393 项单元测试 + 53 项浏览器测试全部通过**。
