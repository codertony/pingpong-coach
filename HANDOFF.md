...
| 几何/滤波/切分/特征/规则/准备区标定/手部几何/分布判据 | motion-core 218 项测试 |
| 后端全链路（含 mock、`.env` 加载与真实模型契约） | api 179 项测试（含 1 项按需 skip 的真实模型烟雾测试）|
| 前端采集/证据/播报/组件渲染/配色对比度 | web 105 项 vitest（含 jsdom + Testing Library 渲染测试） |
| **浏览器真实行为** | **web 72 项 Playwright（真实 Chromium）** |
...
> 🔴 **最重要的一句话**：
> 测试从 172 涨到 683（610 单元 + 73 e2e），但**增量几乎全部落在"代码正确性"上**。
...
| ❌ 声称"测试全绿所以识别准确" | 683 项测试（610 单元 + 73 e2e）里没有一项验证识别准确率 |
...
| `pnpm test` | 只跑单元测试（534 项） |
| `pnpm test:e2e` | 只跑浏览器测试（72 项，真实 Chromium；会自起 vite + 一个真实 API 进程） |
...
| `cp .env.example .env` | 接真实模型：把密钥填进 `.env`（已被 `.gitignore` 忽略）。不填则跑 mock |
| `pnpm eval:replay` | 回放评测（无真实标注时会明确拒绝输出精度数字） |
| `pnpm annotate:tolerance` | 人工标注**要标多准**：IoU ≥ 0.5 换算成毫秒（= 该次挥拍时长的 25%）|
...
> 代码骨架完整，工程护栏齐备，**534 项单元测试 + 72 项浏览器测试全部通过**。
