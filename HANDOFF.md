...
| 后端全链路（含 mock、`.env` 加载与真实模型契约） | api 158 项测试（含 1 项按需 skip 的真实模型烟雾测试）|
...
> 🔴 **最重要的一句话**：
> 测试从 172 涨到 582（510 单元 + 72 e2e），但**增量几乎全部落在"代码正确性"上**。
...
| ❌ 声称"测试全绿所以识别准确" | 582 项测试（510 单元 + 72 e2e）里没有一项验证识别准确率 |
...
> 代码骨架完整，工程护栏齐备，**510 项单元测试 + 72 项浏览器测试全部通过**。
