...
**最近一次全量验证**：`pnpm verify` 退出码 0（393 项单元测试 + 依赖体积预算），`pnpm test:e2e` 52 passed
...
| **浏览器真实行为** | **web 52 项 Playwright（真实 Chromium）** |
...
> 🔴 **最重要的一句话**：
> 测试从 172 涨到 445（393 单元 + 52 e2e），但**增量几乎全部落在"代码正确性"上**。
...
| ❌ 声称"测试全绿所以识别准确" | 445 项测试（393 单元 + 52 e2e）里没有一项验证识别准确率 |
...
| `pnpm test:e2e` | 只跑浏览器测试（52 项，真实 Chromium；会自起 vite + 一个真实 API 进程） |
...
> 代码骨架完整，工程护栏齐备，**393 项单元测试 + 52 项浏览器测试全部通过**。
