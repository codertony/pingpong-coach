...
| `@pingpong/motion-core` | 95 | **169** | +74（退化几何、CV 边界、NaN 约定、准备区标定、手部几何） |
| `@pingpong/api` | 28 | **130** | +102（知识筛选、去重 TTL、prompt 缺失值、provider 不重试、analyze 状态码、模糊） |
| `@pingpong/web`（单元） | 37 | **60** | +23（组件渲染、摄像头错误映射、配色对比度、60fps 调度） |
| `@pingpong/web`（浏览器） | 0 | **50** | +50（真实 Canvas 像素、真实 Worker、真实 ImageBitmap、前后端真实串联 + 红线 8 服务端校验、摄像头故障） |
| **合计** | 172 | **439** | **+267** |
...
> **本轮最重要的一句话**：
> 测试从 172 涨到 439，但**增量几乎全部落在"代码正确性"**上。
