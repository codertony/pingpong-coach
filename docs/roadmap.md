...
| F-043 | 指导人工标注的**联系表分辨率不够**：验收判据（IoU ≥ 0.5 = 单板时长的 25%）要求 800ms 单板标到 **±200ms**，而默认格子是 **250ms** —— 照着标会**系统性低报**，且责任看起来在标注者身上（「谁都没错但结论是错的」）。**第二层**：自检第一版拿**已检出的**挥拍时长当基准，而连续对拉时检测会**合并**（F-022）⇒ 基准偏长、容差偏松，自检会错误地说「够用」。**已修复**：格子可配（`PPC_CONTACT_SHEET_STEP_MS`）、导出时**自动自检**并打印「时长 → 容差」对照表与后果，基准改取 `PPC_EXPECTED_STROKE_MS`（默认 800ms） |
