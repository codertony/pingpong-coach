...
| 前端采集链路 | `[x]` | web 60 项 vitest + 50 项浏览器测试 |
...
**A 类现状**：原清单里只剩 **A4 有"未测项"**，而那一项（真机端到端延迟）**必须有真人真设备**
才能测 —— 它不属于"代码正确性"，被归错类了。A7 是"写了镜像但本机没有 docker"，
A8 确认不适用。其余全部完成。

> 教训（写在这里给下一个人）：A7 与 A2 我都先判过"不做"，后来都做成了。
> 两次的共同点是把**某个工具的限制**当成了**事情本身的限制**
> （"vite 代理改写做不到" → 那就别用 vite 代理；"没有 docker" →
> 镜像的每个环节都可以用等价命令验证）。下次说"做不了"之前，先问一句：
> 是这件事做不了，还是我选的这条路走不通？
