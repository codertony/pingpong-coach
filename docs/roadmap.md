...
| 后端 + Mock 适配器 | `[x]` | api 136 项测试 |
| 前端采集链路 | `[x]` | web 102 项 vitest + 71 项浏览器测试 |
...
| F-036 | 同一件事实（"这具身体看得清吗"）**两处口径不同**：绘制层要求 `score ≥ 0.5`，而 `TrainingSession.findPoint` 只挡 `visible === false`（= `confidence > 0`），于是置信度 **0.01 与 0.99 等价** ⇒ 体尺度、准备区半径、分段用的腕部都可由**绘制层会拒绝的点**决定。**发现方式**：给新写的"真实摄像头链路"用例加反向对照（合成图案、无人），各采样 20 次 —— 真人 **骨架 20/20**，无人 **骨架 1/20 而准备区圆 10/20**：屏幕在画一个它几乎不肯承认看得见的身体的圈。确定性证据见 `apps/web/test/low-confidence-body.test.ts`（躯干点全 0.05 置信度 → 体尺度照样 150px、半径照样 45px）。**未修**：两个改法都会动到真实素材上的行为，而真实素材上只量到过**合成版本**（有人素材 244/244 检出、置信度总体很高，未观察到）→ 按"不在真实数据前调阈值"记为待定，见 `known-failures.md` F-036 的两条待定问题 |
