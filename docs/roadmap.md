...
| F-022 | 连续对拉时**相邻几板被合并成一次挥拍**（少算）。量化后：回位位置落在 **0.29~0.40 体尺度**，正好骑在准备区半径（0.3）边界上，于是每次回位只在区内待 33~67ms，凑不满 `returnStableMinMs = 120`。**哪两个阈值在相互作用已经清楚，但该怎么改必须先有人工标注**（放大半径 / 缩短驻留 / 两级区，三者行为不同） |
