...
**预期**：**`55 passed, 20 skipped`**，约 40 秒。

⚠️ **这里的数字取决于你有没有下模型**（阶段 3 才下，而这一步在它前面）：

| 什么时候跑 | 结果 | 说明 |
| --- | --- | --- |
| 照清单顺序（**还没下模型**） | `55 passed, 20 skipped` | 有 4 条用例需要真实模型资产，缺了整组 skip |
| 下过模型之后再跑（阶段 3 之后） | `59 passed, 16 skipped` | 那 4 条能真的跑起来 |

差的 4 条是：`app.e2e.ts` 的「点开始训练后：引擎真实就绪，练习页拿到实时画面」与
「停止后摄像头轨道被真正关闭」，以及 `camera-fault.e2e.ts` 的两条（这两条更值钱 ——
**只有在整页真链路上才暴露的 F-007 / F-008 只靠它们守**）。
**实测两个数字都验过**（2026-09-17，同一份代码的干净克隆里各跑一遍）。

其余 16 项都是**按需**探针：需要真实素材、真实摄像头或 20 分钟时长 ——
设了对应环境变量才会跑。**逐个对应关系**（这 16 项在 2026-09-17 全部用对应开关跑绿过一次）：

| 开关 | 覆盖的用例（条数）| 本机实测结果 |
| --- | --- | --- |
| `PPC_VERIFY_VIDEO=<素材>` | 10 条：`app.e2e` 1 · `hand-on-footage` 2 · `pose-anatomy` 1 · `pose-overlay` 1 · `segmentation-eval` 3 · `video-playback` 1 · `video-to-analysis` 1 | 全绿 |
| `PPC_PROBE_CAMERA=1` | 2 条：`camera-enumeration` · `camera-device-picker` | 都跑通（后者在设备下拉里选中后拿到 1280×720）|
| `PPC_FAKE_CAMERA_Y4M=<y4m>` | 2 条：`live-capture` 真人 / 合成对照 | 真人骨架 20/20、对照 0/20 |
| `PPC_HAND_IMAGE=<图>` | 1 条：`hand-model` 接口契约 | 1 passed（**要整帧，不要裁剪**）|
| `PPC_SOAK=1`（+`PPC_SOAK_MINUTES`）| 1 条：`soak` 20 分钟 | 队列/吞吐/堆三项都通过 |

> **所以默认输出里的"16 skipped"不是"有 16 项没验过"**，而是"这 16 项的输入不在默认环境里"。
> 它们各自需要的输入上面都写了 —— 想复现哪一条，照着那一行设变量即可。

**退出码应当是 0**：缺资产的用例是 **skip，不是 fail**。它要是红的，
说明遇到的是别的问题（缺资产却报失败会让真信号被假信号淹掉，见 F-065）。
...
- [ ] 断言总数 75 项，且 `通过 + 跳过 = 75`（跳过是 20 还是 16，看这一步有没有下过模型）
- [ ] 退出码是 0（缺资产的用例会 skip，不会红）
...
**摄像头设备选择（可选，走产品界面而不是裸 API）**：

```bash
PPC_PROBE_CAMERA=1 pnpm --filter @pingpong/web test:e2e camera-device-picker
```

它在下拉里**逐台选设备**、点「开始训练」，断言**至少一台**能让练习页的 `<video>`
真的出画面（`videoWidth > 0`），并断言起不来的那些**给出了可操作的错误**而不是静默。
两者分工不同：上面那条回答"哪台设备能起流"，这条回答"**产品接不接得上**"。
实测（2026-09-17）：选 `HIK 2K USB CAMERA` → **1280×720**。

> ⚠️ **同一个设备可能一会儿行一会儿不行**（F-009 已改成"间歇性"）：实测同一台 HIK
> 下午 8005ms 超时、约 1.5 小时后 2926ms 起流。所以**一条命令失败不等于设备坏了** ——
> 换个时刻、或在下拉里换一台设备再试。
