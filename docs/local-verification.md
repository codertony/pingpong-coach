...
**预期**：**`55 passed, 19 skipped`**，约 40 秒。

⚠️ **这里的数字取决于你有没有下模型**（阶段 3 才下，而这一步在它前面）：

| 什么时候跑 | 结果 | 说明 |
| --- | --- | --- |
| 照清单顺序（**还没下模型**） | `55 passed, 19 skipped` | 有 4 条用例需要真实模型资产，缺了整组 skip |
| 下过模型之后再跑（阶段 3 之后） | `59 passed, 15 skipped` | 那 4 条能真的跑起来 |
...
其余 15 项都是**按需**探针：需要真实素材、真实摄像头或 20 分钟时长 ——
...
- [ ] 断言总数 74 项，且 `通过 + 跳过 = 74`（跳过是 19 还是 15，看这一步有没有下过模型）
