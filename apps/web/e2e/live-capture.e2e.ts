...
test.describe("真实摄像头链路 · 文件当摄像头", () => {
  ...
  test("反向对照：合成图案（画面上没有人）上，骨架只允许零星出现", async () => {
    ...
    try {
       ...
       *
       * ⚠️ 这条对照还顺带量到一件**没修的事**：合成图案上
       * **准备区圆稳定画出来（10/20），而骨架只零星出现（1/20）** ——
       * 屏幕在画一个"它几乎不肯承认看得见"的身体的约束圈。
       * 那不是判据失灵，是 F-036 —— 同一件事实（"这具身体看得清吗"）
       * 在 `findPoint`（只挡 `visible === false`）与绘制层（要求 ≥ 0.5）
       * 处口径不同。**这里不修**：改了会动到真实素材上的行为，
       * 而当前没有能证明该改的证据。详见 `docs/known-failures.md` F-036。
       */
      const samples = await sampleOverlay(page, 20, 500);
