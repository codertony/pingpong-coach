...
文档一致性 / 密钥守卫 / 依赖体积预算 / CI），**787 项测试**（713 单元 + 74 浏览器）。
...
> - 真实摄像头下**产品从未取到过流**（[F-009](./docs/known-failures.md)，OPEN）。**根因已定位**（2026-09-17 真机探针）：本机 8 个视频设备里 **3 台能出流**，产品取的是**默认设备**，而默认那台 `HIK 2K USB CAMERA` 起不来（8.0s 超时）—— 所以这既不是"机器没有摄像头"，也不是"浏览器不让用"；
...
> **默认一跑通过的不是全部。** `api` 有 1 项、浏览器有 15 项是**按需跳过**的探针
> （需要真实素材、真实摄像头或 20 分钟时长），所以 `pnpm test:e2e` 的默认输出是
> `59 passed, 15 skipped`。这不是失败，也**不是**"全绿"——它们的证据要你本机的素材和设备才能产生。
