...
> **关于"识别准不准"，能说的是这些**（按证据强度分开写，别把它们混成一句）：
>
> - **没有任何 accuracy 数字**：「第一份 precision / recall」需要人对着视频逐板标两端，
>   而 `evaluation/` 里还没有这样的真值 ⇒ 指标**算不出来**（`pnpm eval:replay` 会明确拒绝出数，
>   而不是编一个）；
> - **但已有反向证据，而且不小**：在一支 8.15 秒素材上，独立视觉复核（逐格看联系表，非算法信号）
>   读出挥拍数量级是 **8~11 次**，而分段器报了 **2 次** —— **一个数量级的少报**（F-022 的量化，
>   见 `docs/evaluation-log.md` 事实 8）。所以"识别准不准"不是"未知"，是"已知有一处差一个数量级"；
> - 骨架贴合只在**一支素材、一个人、一个机位**上目视确认过一次（判定者还是看图模型）；
> - 手部：检出率 **2/31 帧**，其中一次是**误检**（画在了人脸/下巴上），而该模型不提供逐点置信度；
> - 真实摄像头下**产品已能取到画面**（2026-09-17 实测：设备下拉里选中后 1280×720）——
>   但同一台设备也出现过 Chrome 自身 8s 超时不起流，所以 [F-009](./docs/known-failures.md) 仍标 **OPEN（间歇性）**；
...
> 权重由 `pnpm models:fetch` 下载并校验 sha256；WASM **由构建自己铺**（起 `vite` / `vite build` 时
> 那个插件会从 `node_modules` 复制进 `public/wasm`，不需要手动复制）。所以**第 3 步不能跳过**。
>
...
> `59 passed, 16 skipped`。这不是失败，也**不是**"全绿"——它们的证据要你本机的素材和设备才能产生。
...
3. 跑分段评估，**三步缺一不可**（`evaluation/samples.json` 的 `$howToEvaluate` 里有完整说明）：
   ...
   **事件容差按源帧率推导**（默认 2 个源帧：60fps→33ms、30fps→67ms），脚本会把用到的
   容差打印出来；要换别的容差用 `PPC_EVENT_TOLERANCE_MS` 覆盖。**刻意不给默认毫秒数**：
   有默认值，「容差是多少」这件事就会从报告里消失，指标也就无从追责。

   指标分两层且**刻意不合并**：**分段**（这一板有没有被找到：IoU / precision / recall）与
