...
> ⚠️ **这条链路上有一处目前是断的**：`EvidencePacket` 里的 `keyframes` **恒为空数组**
> —— `KeyframeCache` 从来没有任何产品代码往里放图（写入者只在测试里）。
> 所以"多模态调用"**实际收到的是纯文本**。链路其余部分（切分、特征、证据、校验）是通的，
> 只有图片这一段没接。见 [F-028](./docs/known-failures.md)。
> 会话现在会**如实提示**"本组证据不含图片"，不再静默。
...
当前共 **513 项测试**（448 单元 + 65 浏览器）：

| 包 | 单元测试 | 浏览器测试 |
| --- | --- | --- |
| `@pingpong/contracts` | 30 | — |
| `@pingpong/motion-core` | 207 | — |
| `@pingpong/api` | 135 | — |
| `@pingpong/web` | 76 | 65 |
| **合计** | **448** | **65** |
...
- 真实模型的延迟与建议质量（而且**证据包里没有图片** → 模型只看到文本，见 F-028）；
- **关键帧图片链路** —— 整个缓存没有任何产品代码写入，`keyframes` 恒为空数组（F-028）。会话会提示这一点，但功能本身未实现；
